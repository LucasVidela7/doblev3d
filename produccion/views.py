...
def _slots_ams_bambu(estado_bambu):
    """
    Normaliza la telemetría AMS del bridge.

    Según la versión del bridge/firmware, AMS puede persistirse como:
    - {"ams": [{...}]}
    - [{...}]
    - una unidad AMS directa
    - una estructura anidada adicional

    También recupera el AMS desde payload crudo si el campo dedicado
    quedó vacío. Devuelve (slots, tray_now), donde cada slot conserva
    ams_id, tray_id y la bandeja original.
    """
    payload = (
    ...
    try:
        with transaction.atomic():
            ...
            if existente:
                ...
                # La restricción de base permite un único archivo principal
                # por producto + cantidad. Bajamos la versión anterior antes
                # de crear la nueva dentro de la misma transacción.
                if anterior:
        ...
        if archivo_nuevo_path:
            try:
                nuevo.archivo.storage.delete(
                    archivo_nuevo_path
                )
            except Exception:
                logger.exception(
                    "No se pudo limpiar archivo G-code huérfano %s",
                    archivo_nuevo_path,
                )

        logger.exception(
            "Error asociando G-code desde Producción. produccion_id=%s nombre=%s",
            produccion_id,
            getattr(
                archivo,
                "name",
        ...
        messages.error(
            ...
            (
                ...
                "El error quedó registrado para diagnóstico."
            ),
        )
